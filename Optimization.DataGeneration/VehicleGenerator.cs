...
namespace Optimization.DataGeneration
{
    ...
    public static class VehicleGenerator
    {
        ...
        public static IEnumerable<Vehicle> GenerateUniqueVehicle(int count)
        {
            for (int i = 0; i < count; i++)
                yield return GenerateVehicle();
        }

        /// <summary>
        /// Генерирует транспорт.
        /// </summary>
        /// <returns>Экземпляр <see cref="Vehicle"/></returns>
        private static Vehicle GenerateVehicle()
        {
            ...
            return new Vehicle(capacity, accelerationTime, dimension, vehicleType, price);
        }
